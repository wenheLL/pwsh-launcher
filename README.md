...
- 右边命令优先读项目根目录的 `.pwsh-launcher.json`；没有该文件时再读 `package.json` 的 `scripts` 生成，常用脚本排前面（dev → build → test → typecheck → lint → dist:win → dist → …），最多 14 条
- 两者都没有就退回 `git status -sb` / `git pull --ff-only`
- 选中命令后双击或回车 → 新窗口里 `cd` 到该目录并预填命令，**按回车才执行**，执行前还能改
- **内置终端**（默认）：会话长在启动器自己的标签页里，不需要 Windows Terminal，任务栏始终只有一个按钮
- 标签栏是**仿 Chrome 的自绘样式**：浅色栏底（跟随系统强调色，跟 Chrome 自己的标签栏同色）+ 白色圆角活动标签 + 未选中标签之间的细分隔线，没有原生那种 3D 边框和阴影
- 终端标签**自带 ✕ 关闭按钮**（悬停时背后出现圆形底色），标签做得比较大好点；完整路径放在标签的悬停提示里
...
勾上「以管理员身份（UAC）」再点「打开 pwsh」，就会用 `Start-Process -Verb RunAs` 提权打开 —— 也就是资源管理器右键里的"以管理员身份运行"：**会弹 UAC，然后在一个独立窗口里**打开，选中的命令照样预填到提示符上。

**为什么管理员会话进不了标签页（不是偷懒，是 Windows 的设计）**

- 管理员会话 = 高完整性级别的进程。Windows 不给已经在跑的进程"就地提权"，唯一办法是让 ShellExecute 用 `runas` **另起一个新进程**（那个 UAC 弹窗就是它）。
- 而内嵌终端是我们自己 `CreatePseudoConsole` 造出来的：**非提权父进程创建不出提权子进程**，ConPTY 也跨不过完整性级别。两件事天生互斥。
- 这也顺带解释了"管理员窗口看起来不一样"：它属于另一个进程上下文，任务栏分组跟普通窗口是分开的。

所以按需勾选最划算：平时不勾（会话都在标签页里），偶尔要提权时勾上点一次。

如果你希望**所有**会话都是管理员（比如整个工作流都要提权）：右键桌面上的启动器快捷方式 →「以管理员身份运行」，或者快捷方式属性 →「高级」→ 勾上"用管理员身份运行"。那样启动器自己就是提权进程，它内嵌的所有会话都会是提权的；代价是启动器全程带着管理员权限跑，而且每次启动（包括开机自启）都会弹 UAC。

（想连 UAC 弹窗都省掉还要内嵌，只剩一条路：先提权起一个常驻 helper 进程当命名管道服务端，由它在提权上下文里建 ConPTY、把字节流传回启动器。代价是长期常驻一个管理员进程，本项目不做。）
...
项目自定义命令放在项目根目录的 `.pwsh-launcher.json`：

```json
{
  "commands": [
    { "label": "启动游戏", "command": ".\\start_game.bat" },
    { "label": "打开编辑器", "command": ".\\start_editor.bat" },
    "git status -sb"
  ]
}
```

- `commands` 里可以混用字符串和 `{ label, command }` 对象。
- `label` 是列表里显示的文字，`command` 是终端里预填的命令。
